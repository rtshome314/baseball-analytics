...
with st.sidebar:
    ...
    season = st.selectbox("Season", [2026, 2025, 2024, 2023], index=1)
...
if not trad_stats.empty:
    ...
    with st.expander("📌 Bookmarks"):
        ...
        if st.button("💾 Save Bookmark") and new_bookmark_name:
            if "bookmarked_players" in st.session_state:
                bookmarks[new_bookmark_name] = st.session_state["bookmarked_players"]
    ...
    default_players = st.session_state.get("bookmarked_players", [])
    default_players = [p for p in default_players if p in available_players]
    ...
    selected_players = st.multiselect(
        ...
        default=[p for p in default_players if p in player_options],
        placeholder="Type to search players...",
    )

    # Keep session state updated for saving
    st.session_state["bookmarked_players"] = selected_players
