...
st.markdown("Compare up to **10 players** side-by-side with percentile rankings.")
...
if not trad_stats.empty:
    ...
    with st.expander("📌 Bookmarks"):
        ...
        if bookmark_names:
            ...
            if st.button("📂 Load Bookmark") and chosen_bookmark:
                st.session_state["bookmarked_players"] = bookmarks[chosen_bookmark]
                st.session_state["bookmark_just_loaded"] = True
                st.session_state["players_initialized"] = False
                st.rerun()
        else:
    ...
    # Only use bookmarked_players as default when explicitly loaded, then clear it
    default_players = []
    if "bookmarked_players" in st.session_state:
        default_players = [p for p in st.session_state["bookmarked_players"] if p in player_options]
        # Clear after consuming so it doesn't persist across reruns
        if "bookmark_just_loaded" in st.session_state and st.session_state["bookmark_just_loaded"]:
            st.session_state["bookmark_just_loaded"] = False
        else:
            # Only use as default once, then let multiselect manage its own state
            if "players_initialized" not in st.session_state:
                st.session_state["players_initialized"] = True
            else:
                default_players = []
    ...
    # Update bookmarked_players only when saving
...
# --- Split Filters ---
        ...
        view_mode = st.radio("View Mode", ["Percentile Charts", "Radar Comparison", "Stats Table"], horizontal=True)
        

        if view_mode == "Percentile Charts":
            for i in range(0, len(selected_players), 5):
                cols = st.columns(5)
                for j, col in enumerate(cols):
                    idx = i + j
                    if idx >= len(selected_players):
                        break
                    pname = selected_players[idx]
                    with col:
                        player_pcts = {}
                        player_raw = {}
                        ...
                        if pct_split_active:
                            ...
                            if available:
                                qual_tag = " ✅" if pname in qualified_players else ""
                                fig = create_percentile_chart(pname + qual_tag, player_pcts, available, raw_values=player_raw)
                                st.plotly_chart(fig, use_container_width=True)
                            else:
